...
class ClientRequestWatcher extends Watcher
{
    ...
    {
        if (! LaravelAppCast::isRecording()) {
            return;
        }

        LaravelAppCast::recordClientRequest(IncomingEntry::make([
        ...
        if (is_string($content)) {
            if (is_array(json_decode($content, true)) &&
                ...
                return $this->contentWithinLimits($content)
                        ? $this->hideParameters(json_decode($content, true), LaravelAppCast::$hiddenResponseParameters)
                        : 'Purged By Telescope';
            }
    ...
    protected function headers($headers)
    {
        ...
        return $this->hideParameters($headers,
            LaravelAppCast::$hiddenRequestHeaders
        );
    }
    ...
    protected function payload($payload)
    {
        return $this->hideParameters($payload,
            LaravelAppCast::$hiddenRequestParameters
        );
    }
