...
class EventWatcher extends Watcher
{
    ...
    {
        if (! LaravelAppCast::isRecording() || $this->shouldIgnore($eventName)) {
            return;
        }

        $formattedPayload = $this->extractPayload($eventName, $payload);

        LaravelAppCast::recordEvent(IncomingEntry::make([
            'name' => $eventName,
    ...
    protected function shouldIgnore($eventName)
    {
        return $this->eventIsIgnored($eventName) ||
            (LaravelAppCast::$ignoreFrameworkEvents && $this->eventIsFiredByTheFramework($eventName));
    }
